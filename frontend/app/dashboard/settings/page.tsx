...
export default function SettingsPage() {
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto p-8 md:p-12 lg:p-16 min-h-screen relative">
      ...
      <main className="w-full">
        <div className="max-w-[1000px] mx-auto">
          ...
          <motion.div {...fadeInUp} className="mb-16">
            <h1 className="text-4xl font-semibold tracking-tight tracking-tighter mb-2">Preferences</h1>
            <p className="text-white/20 text-xs font-semibold tracking-tight uppercase tracking-[0.2em]">Clinical Environment · Identity · Global Connectivity</p>
          </motion.div>
              ...
              <AnimatePresence mode="wait">
                {activeTab === 'profile' && (
                  <motion.div key="profile" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                    <section>
                      <h3 className="text-xs font-semibold tracking-tight uppercase tracking-[0.2em] text-white/20 mb-6">Credential Presentation</h3>
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <label className="text-[10px] font-semibold tracking-tight uppercase tracking-widest text-white/10 ml-1">Legal Full Name</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-semibold tracking-tight uppercase tracking-widest text-white/10 ml-1">Practice Location</label>
                ...
                {activeTab === 'api' && (
                  <motion.div key="api" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                    <section>
                      <h3 className="text-xs font-semibold tracking-tight uppercase tracking-[0.2em] text-white/20 mb-6">Bridging Credentials</h3>
                      <div className="space-y-6">
                        <div className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] p-8 border-white/5 group">
                          <div className="flex items-center gap-3 mb-4">
                            <div className="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-400">
                              <Globe size={16} />
                            </div>
                            <span className="text-[11px] font-semibold tracking-tight uppercase tracking-widest">WHO ICD-11 Platform Key</span>
                          </div>
                          <div className="flex items-center gap-4 bg-black/40 border border-white/5 rounded-2xl p-2 pl-6">
                            ...
                            <button className="px-6 py-3 rounded-xl bg-white text-black text-[10px] font-semibold tracking-tight uppercase tracking-widest">Rotate</button>
                          </div>
                ...
                {activeTab === 'appearance' && (
                  <motion.div key="appearance" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                    <section>
                      <h3 className="text-xs font-semibold tracking-tight uppercase tracking-[0.2em] text-white/20 mb-6">Interface Dynamics</h3>
                      <div className="grid grid-cols-3 gap-4">
                         {[
                           ...
                           >
                              ...
                              <span className="text-[10px] font-semibold tracking-tight uppercase tracking-widest">{mode.label}</span>
                ...
                {activeTab === 'security' && (
                  <motion.div key="security" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                    <section className="space-y-4">
                      <h3 className="text-xs font-semibold tracking-tight uppercase tracking-[0.2em] text-white/20 mb-6">Encryption & Guarding</h3>
                      {[
