        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          <motion.div {...fadeInUp} className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-4xl font-semibold tracking-tight tracking-tighter mb-1">Infrastructure Control</h1>
                <p className="text-white/30 text-xs font-semibold tracking-tight uppercase tracking-[0.2em] leading-relaxed italic">ISO 22600 · Blockchain Ledger Active</p>
              </div>
            </div>
            <motion.div whileHover={{ scale: 1.02 }} className="flex items-center gap-3 px-6 py-3 bg-red-500/5 rounded-2xl border border-red-500/20 backdrop-blur-xl group cursor-help">
              ...
              <span className="text-[11px] font-semibold tracking-tight uppercase tracking-widest text-red-100 group-hover:text-red-400 transition-colors">Security Score: {SYSTEM_STATS.securityScore}/100</span>
            </motion.div>
          </motion.div>
            ...
            {[
              ...
              return (
                <motion.div 
                  ...
                  key={i} className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] p-8 group hover:border-white/20 transition-all cursor-default"
                >
                  ...
                  <div className="text-3xl font-semibold tracking-tight tracking-tighter mb-1" style={{ color: stat.color }}>{stat.value}</div>
                  <div className="text-[9px] font-semibold tracking-tight uppercase tracking-[0.2em] text-white/10">{stat.label}</div>
                </motion.div>
              );
            })}
          </div>
          ...
          <motion.div {...fadeInUp} transition={{ delay: 0.4 }} className="flex p-1.5 bg-white/[0.03] rounded-[24px] border border-white/5 mb-10 w-fit">
            {(['audit', 'users', 'system'] as const).map(tab => (
              <button 
                ...
                className={`px-8 py-3 rounded-2xl text-[10px] font-semibold tracking-tight uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-black shadow-lg shadow-white/10' : 'text-white/20 hover:text-white/40'}`}
              >
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'audit' && (
              ...
              >
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                    {['all', 'READ', 'CREATE', 'WRITE', 'DELETE', 'LOGIN'].map(f => (
                      <button key={f} onClick={() => setActionFilter(f)} className={`px-4 py-2 rounded-xl text-[9px] font-semibold tracking-tight uppercase tracking-widest border transition-all ${actionFilter === f ? 'bg-white text-black border-transparent' : 'text-white/20 border-white/10 hover:border-white/20'}`}>
                        {f}
                      </button>
                    ))}
                  </div>
                ...
                {/* Audit Stream */}
                <div className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] overflow-hidden rounded-[32px] border-white/5">
                  <div className="overflow-x-auto scrollbar-hide">
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-white/5">
                          {['Sequence', 'Event Node', 'Entity', 'Identity', 'Security Status', 'Hash Commit'].map(h => (
                            <th key={h} className="text-left px-8 py-6 text-[9px] font-semibold tracking-tight uppercase tracking-[0.2em] text-white/10">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {filteredLogs.map((log, i) => (
                          ...
                          >
                            <td className="px-8 py-5">
                              ...
                              <div className="text-[8px] text-white/10 font-semibold tracking-tight uppercase mt-1">{new Date(log.timestamp).toLocaleDateString()}</div>
                            </td>
                            <td className="px-8 py-5">
                              <span className={`text-[9px] font-semibold tracking-tight uppercase px-3 py-1.5 rounded-xl border ${ACTION_COLORS[log.action] || 'text-white/40 bg-white/5 border-white/10'}`}>{log.action}</span>
                            </td>
                            <td className="px-8 py-5">
                              <div className="text-[12px] font-semibold tracking-tight tracking-tight">{log.resource}</div>
                              <div className="text-[8px] text-white/20 font-mono mt-1 opacity-40">{log.resourceId}</div>
                            </td>
                            <td className="px-8 py-5">
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-[10px] font-semibold tracking-tight">{log.user[0]}</div>
                                <div>
                                  <div className="text-[12px] font-bold tracking-tight">{log.user}</div>
                                  <div className={`text-[8px] font-semibold tracking-tight uppercase mt-0.5 ${log.role === 'admin' ? 'text-red-400' : 'text-white/30'}`}>{log.role}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-8 py-5">
                              <div className={`flex items-center gap-1.5 text-[9px] font-semibold tracking-tight uppercase ${log.status === 'success' ? 'text-[#00d69b]' : 'text-red-400'}`}>
              ...
              >
                {USERS.map((user, i) => (
                  <motion.div 
                    ...
                    key={i} className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] p-8 group hover:border-white/10 transition-all"
                  >
                    <div className="flex items-start justify-between mb-8">
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-center font-semibold tracking-tight text-xl text-white/20 group-hover:text-white transition-colors">
                          {user.name[0]}
                        </div>
                        <div>
                          <div className="text-xl font-semibold tracking-tight tracking-tighter">{user.name}</div>
                          <div className="flex items-center gap-2 mt-1">
                             <span className={`text-[9px] font-semibold tracking-tight uppercase px-2 py-0.5 rounded-md border ${user.role === 'admin' ? 'text-red-400 border-red-400/20 bg-red-400/5' : 'text-blue-400 border-blue-400/20 bg-blue-400/5'}`}>{user.role}</span>
                             <span className="text-[9px] font-semibold tracking-tight uppercase text-white/20 tracking-widest">{user.lastActive} active</span>
                          </div>
                        </div>
                      </div>
                      <div className={`flex items-center gap-1.5 text-[9px] font-semibold tracking-tight uppercase ${user.status === 'active' ? 'text-[#00d69b]' : 'text-white/20'}`}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                      {user.permissions.map(p => (
                        <span key={p} className="px-3 py-1 rounded-lg bg-white/[0.03] border border-white/5 text-[8px] font-semibold tracking-tight uppercase tracking-widest text-white/30">{p}</span>
                      ))}
                    </div>

                    <div className="flex gap-3">
                      <button className="flex-1 py-3 rounded-xl glass border border-white/5 text-[9px] font-semibold tracking-tight uppercase tracking-widest hover:bg-white/5 transition-all">Revoke Token</button>
                      <button className="flex-1 py-3 rounded-xl bg-white text-black text-[9px] font-semibold tracking-tight uppercase tracking-widest shadow-xl shadow-white/5 transition-all">Adjust RBAC</button>
                    </div>
                  </motion.div>
                ))}
              </motion.div>
            ...
            {activeTab === 'system' && (
              ...
              >
                <div className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] p-10 flex flex-col justify-center gap-8 relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-10 opacity-[0.02] group-hover:opacity-[0.05] transition-opacity">
                    <Server size={180} />
                  </div>
                  <div>
                    <div className="text-[10px] font-semibold tracking-tight uppercase tracking-[0.3em] text-white/10 mb-2">Primary Node Uptime</div>
                    <div className="text-6xl font-semibold tracking-tight tracking-tighter text-[#00d69b]">{SYSTEM_STATS.uptime}</div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  {[
                    ...
                    <div key={i} className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] p-6">
                      ...
                      <div className="text-[8px] font-semibold tracking-tight uppercase tracking-[0.2em] text-white/10 mt-1">{s.label}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="col-span-full glass p-10 relative overflow-hidden group">
                  <div className="flex items-center justify-between mb-10">
                    <h3 className="text-[10px] font-semibold tracking-tight uppercase tracking-[0.3em] text-white/20 flex items-center gap-3">
                      <Hash size={14} className="text-red-400" /> Transparent Blockchain Accountability Chain
                    </h3>
                    <div className="text-[9px] font-semibold tracking-tight uppercase text-red-400 animate-pulse">Live Ledger Update</div>
                  </div>
                  ...
                  <div className="flex items-center gap-6 overflow-x-auto pb-6 scrollbar-hide">
                    {MOCK_AUDIT_LOGS.slice(0, 5).map((log, i) => (
                      <div key={i} className="flex items-center gap-6 shrink-0">
                        ...
                        >
                          <div className="absolute -top-2 -right-2 w-6 h-6 rounded-lg bg-red-500/10 border border-red-500/20 flex items-center justify-center text-[9px] font-semibold tracking-tight text-red-400 italic">#{i}</div>
                          ...
                          <div className="text-[9px] text-white/20 font-semibold tracking-tight uppercase tracking-widest">{log.user}</div>
                        </motion.div>
