...
function CodeBadge({ code, label, color }: { code: string; label: string; color: string }) {
  ...
  return (
    ...
    >
      <span className="font-mono font-semibold tracking-tight text-[13px]">{code}</span>
      <span className="text-[10px] font-bold uppercase tracking-widest opacity-30">— {label}</span>
      <span className="ml-auto text-[9px] font-semibold tracking-tight uppercase text-white/20 tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity">{copied ? 'Copied' : 'Copy'}</span>
    </motion.div>
  );
}
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          <motion.div {...fadeInUp} className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-4xl font-semibold tracking-tight tracking-tighter mb-1">Dual Coding Hub</h1>
                <p className="text-white/30 text-xs font-semibold tracking-tight uppercase tracking-[0.2em] leading-relaxed">NAMASTE ↔ ICD-11 ↔ TM2 Concept Mapping</p>
              </div>
            </div>
          ...
          <motion.div {...fadeInUp} transition={{ delay: 0.1 }} className="relative mb-12 group">
             ...
             <div className="relative glass p-2 rounded-[30px] border border-white/10 flex items-center gap-2 focus-within:border-[#00d69b]/40 transition-all bg-black/40">
               ...
               <motion.button
                 ...
                 className="px-8 py-4 rounded-[22px] bg-[#00d69b] text-black font-semibold tracking-tight text-xs uppercase tracking-widest shadow-lg"
               >
                 Compute
               </motion.button>
             </div>
                   ...
                   {suggestions.map((s, i) => (
                     ...
                     >
                       ...
                       <div className="flex-1">
                         ...
                         <div className="text-[10px] text-white/20 font-semibold tracking-tight uppercase tracking-widest">{s.icd11_name}</div>
                       </div>
                       <div className="flex items-center gap-2">
                         <span className="text-[9px] font-semibold tracking-tight tracking-widest bg-[#00d69b]/10 text-[#00d69b] px-2 py-1 rounded-md">{s.namaste_code}</span>
                       </div>
                     </button>
                   ))}
                 </motion.div>
          ...
          <motion.div {...fadeInUp} transition={{ delay: 0.2 }} className="flex items-center gap-4 mb-10 overflow-x-auto pb-2 scrollbar-hide">
            {(['all', 'namaste', 'icd11'] as const).map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-6 py-2 rounded-full text-[10px] font-semibold tracking-tight uppercase tracking-[0.2em] transition-all border whitespace-nowrap ${filter === f ? 'bg-white text-black border-transparent' : 'text-white/20 border-white/10 hover:border-white/20 hover:text-white/40'}`}
              >
                {f === 'all' ? 'Unified Registry' : f === 'namaste' ? 'AYUSH Core' : 'ICD-11 Bridge'}
              </button>
            ))}
            <div className="ml-auto text-[10px] font-semibold tracking-tight uppercase tracking-widest text-white/10">{displayResults.length} Node Matches</div>
          </motion.div>
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="text-[10px] font-semibold tracking-tight text-white/10 transition-colors group-hover/card:text-[#00d69b]/40">{(r.confidence! * 100).toFixed(0)}%</div>
                      </div>
                      <div className="font-semibold tracking-tight text-lg tracking-tighter mb-1 line-clamp-1">{r.namaste_name}</div>
                      <div className="text-[10px] font-semibold tracking-tight uppercase tracking-widest text-white/20 line-clamp-1 mb-4 opacity-60">{r.icd11_name}</div>
                      <div className="flex gap-2">
                        <span className="text-[8px] font-semibold tracking-tight tracking-widest bg-white/[0.03] border border-white/10 px-2 py-1 rounded text-white/30">{r.namaste_code}</span>
                        <span className="text-[8px] font-semibold tracking-tight tracking-widest bg-white/[0.03] border border-white/10 px-2 py-1 rounded text-white/30">{r.icd11_code}</span>
                      </div>
                    </div>
                  </motion.button>
                ))}
              ...
              <AnimatePresence mode="wait">
                {selected ? (
                  <motion.div 
                    ...
                    className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] rounded-[40px] p-10 md:p-12 sticky top-8 border-white/10 overflow-hidden"
                  >
                    ...
                    <div className="relative z-10">
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-12">
                        <div>
                          <div className="flex items-center gap-3 mb-6">
                            <div className="px-3 py-1 rounded-full bg-[#00d69b]/10 border border-[#00d69b]/20 text-[#00d69b] text-[9px] font-semibold tracking-tight uppercase tracking-widest">{selected.category}</div>
                            <div className="w-1 h-1 rounded-full bg-white/10" />
                            <div className="text-[9px] font-semibold tracking-tight uppercase tracking-widest text-white/20 italic">Node Integrity Verified</div>
                          </div>
                          <h2 className="text-5xl font-semibold tracking-tight tracking-tighter mb-2">{selected.namaste_name}</h2>
                        ...
                        <div className="flex flex-col items-center justify-center w-24 h-24 rounded-[32px] border-2 border-[#00d69b]/20 bg-black/40 backdrop-blur-3xl shrink-0">
                          <span className="text-3xl font-semibold tracking-tight text-[#00d69b]">{(selected.confidence! * 100).toFixed(0)}</span>
                          <span className="text-[9px] font-semibold tracking-tight uppercase tracking-widest text-white/20">Reliability</span>
                        </div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
                        <div className="space-y-4">
                           <div className="flex items-center gap-2 px-1 text-[10px] font-semibold tracking-tight text-white/20 uppercase tracking-widest"><Leaf size={14} className="text-[#00d69b]" /> AYUSH Code</div>
                           <CodeBadge code={selected.namaste_code!} label="NAMASTE" color="text-[#00d69b]" />
                        </div>
                        <div className="space-y-4">
                           <div className="flex items-center gap-2 px-1 text-[10px] font-semibold tracking-tight text-white/20 uppercase tracking-widest"><Globe2 size={14} className="text-[#7075ff]" /> Bridging Term</div>
                           <CodeBadge code={selected.tm2_code!} label="TM2 BRIDGE" color="text-[#7075ff]" />
                        </div>
                        <div className="space-y-4">
                           <div className="flex items-center gap-2 px-1 text-[10px] font-semibold tracking-tight text-white/20 uppercase tracking-widest"><FlaskConical size={14} className="text-amber-400" /> WHO MMS</div>
                      ...
                      <div className="p-8 rounded-[32px] bg-white/[0.01] border border-white/5 mb-10 group/map">
                        <div className="text-[10px] font-semibold tracking-tight uppercase tracking-[0.3em] text-white/10 mb-8 text-center italic">Cross-System Concept Mapping</div>
                        <div className="flex items-center justify-between max-w-md mx-auto relative px-4">
                          ...
                          <div className="text-center relative z-10 transition-transform group-hover/map:scale-105">
                            ...
                            <div className="font-mono text-[10px] font-semibold tracking-tight text-white/40">{selected.namaste_code}</div>
                          </div>
                          ...
                          <div className="text-center relative z-10 transition-transform group-hover/map:scale-105">
                            ...
                            <div className="font-mono text-[10px] font-semibold tracking-tight text-white/40">{selected.tm2_code}</div>
                          </div>
                          ...
                          <div className="text-center relative z-10 transition-transform group-hover/map:scale-105">
                            ...
                            <div className="font-mono text-[10px] font-semibold tracking-tight text-white/40">{selected.icd11_code}</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-6">
                        ...
                        <motion.button 
                          whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                          className="flex-1 py-5 rounded-[24px] bg-[#00d69b] text-black font-semibold tracking-tight text-[10px] uppercase tracking-widest shadow-xl shadow-[#00d69b]/10 transition-all flex items-center justify-center gap-3"
                        >
                  ...
                  <motion.div 
                    ...
                    className="bg-white/[0.015] border border-white/[0.03] shadow-[inset_0_1px_rgba(255,255,255,0.02)] rounded-[32px] h-[600px] rounded-[40px] flex flex-col items-center justify-center text-center p-16 border-dashed border-white/5 bg-transparent"
                  >
                    ...
                    <h4 className="text-xl font-semibold tracking-tight tracking-tight mb-4">Registry Standby</h4>
